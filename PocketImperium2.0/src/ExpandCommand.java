...
class ExpandCommand extends Command {
    ...
    public void execute(Player player, Board board, Game game) {
        ...
        if (player.isAI()) {
            // AI automatically chooses sectors and places ships
            System.out.println(player.getName() + " (AI) is placing ships automatically.");

            // AI can prioritize sectors with fewer ships or choose randomly
            for (int i = 0; i < shipsToPlace; i++) {
                // AI picks a random sector from the controlled sectors
                Sector targetSector = controlledSectors.get(new Random().nextInt(controlledSectors.size()));

                // Add a new ship to the selected sector
                targetSector.getShips().add(new Ship(player));
                player.useShips(1);
                System.out.println("AI placed 1 ship in " + targetSector.getName() + ".");
                System.out.println("Ships in supply : " + player.getSupply() + ".");
            }
        } else {
