...
class ExploreCommand extends Command {
    ...
    public void execute(Player player, Board board, Game game) {
        ...
        if (player.isAI()) {
            // AI performs the exploration command automatically
            executeAI(player, board);
        } else {
    ...
    // AI performs the exploration command
    private void executeAI(Player player, Board board) {
        Random random = new Random();
        int fleetsToMove = (player.getCommandAt(board.getCurrentStep()) instanceof ExploreCommand) ? 3 : 1;

        for (int i = 0; i < fleetsToMove; i++) {
            // Randomly select a controlled sector with ships
            List<Sector> controlledSectors = board.getSectors().stream()
                    .filter(sector -> sector.getOwner() == player && !sector.getShips().isEmpty())
                    .toList();

            if (controlledSectors.isEmpty()) return;

            Sector fromSector = controlledSectors.get(random.nextInt(controlledSectors.size()));

            // Randomly choose the number of ships to move (between 1 and the number of ships in the sector)
            int shipsToMove = random.nextInt(fromSector.getShips().size()) + 1;

            // Randomly choose a valid target sector (adjacent and considering Level 3 restrictions)
            Sector toSector = selectRandomTargetSector(fromSector, player, board, random);
            ...
            System.out.println(player.getName() + " moved " + shipsToMove + " ships from " + fromSector.getName() + " to " + toSector.getName() + ".");
    ...
    private Sector selectRandomTargetSector(Sector fromSector, Player player, Board board, Random random) {
        List<Sector> possibleTargets = getAdjacentSectors(fromSector, board, player);

        if (possibleTargets.isEmpty()) return null;

        // Randomly choose a target sector
